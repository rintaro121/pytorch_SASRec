...
#     future_masks = tile(future_mask_const[:seq_len, :seq_len], (batch_size, 1, 1))
...
#     diag_masks = tile(seq_diag_const[:seq_len, :seq_len], (batch_size, 1, 1))
...
class SASRec(nn.Module):
    def __init__(self, num_items, hidden_units, max_len, num_heads, num_layers, dropout_rate, device):
        ...
        # self.causal_mask = create_causal_mask(self.max_len, self.device)
    ...
    def forward(self, input_ids, pos_ids, neg_ids):
        ...
        mask = create_causal_mask(seq_len, self.device)
    ...
    def predict(self, input_ids):
        ...
        item_embeddings = self.item_emb(input_ids) * np.sqrt(self.hidden_units) + self.pos_emb(position_ids)
        mask = create_causal_mask(seq_len, self.device)
        output = self.encoder(item_embeddings, mask)
        return output
